...
class SoundSource(object):
   ...
   #can name it deviation      
  

    def azimuth_angle(self, ref_point):

        a = self.images[:2,:] - ref_point[:2, np.newaxis]
        sign = np.sign(self.images[1,:] - ref_point[1])
        
        angle = np.round(sign*np.rad2deg(np.arccos(a[0,:]/(np.linalg.norm(a, axis=0)))))
    ...
    def elevation_angle(self, ref_point):
    
        d = np.sqrt(np.sum((self.images - ref_point[:, np.newaxis])**2, axis=0))
        a = self.images[2,:] - ref_point[2, np.newaxis]
        f = np.sqrt(a**2+d**2)
        
        sign = np.sign(self.images[2,:] - ref_point[2])
        
        elevation_angle = np.round(sign*np.rad2deg(np.arccos(d/f)))

        for i in range(elevation_angle.shape[0]):
            if (elevation_angle[i] < 0):
                elevation_angle[i] += 360 
    ...
    def find_nearest(self, array, value):
        '''
        Finds the nearest value of our value in array
        ----------
        array:  ndarray 
            The number of points in the second dimension
        value: float
            The value we search for its nearest point

        Returns
        -------
        float
            the nearest value in array
        '''
    ...
    def get_rir_hrtf(self, mic, visibility, Fs, angles, m, t0=0., hrtf=None, t_max=None):
        '''
        Compute the room impulse response between the source
        and the microphone with hrtf whose position is given as an
        argument.
        m is mic_pos
        hrtf is the total data of transfer functions
        '''
        ...
        for i in range(time.shape[0]):
            ...
            ang = self.find_nearest(angles, angle[i])

            # find index of nearest detected angle in angles array
            idx = np.asscalar(np.argwhere(angles==ang))

           
            if visibility[i] == 1:
                time_ip = int(np.round(Fs * time[i]))
                time_fp = (Fs * time[i]) - time_ip                
                ir[time_ip-fdl2:time_ip+fdl2+1+hrtf.shape[0]-1] += np.convolve(alpha[i]*fractional_delay(time_fp), hrtf[:, idx, m])
                if(time_ip+fdl2+1+hrtf.shape[0]-1 > maximum):
                    maximum = time_ip+fdl2+1+hrtf.shape[0]-1           
